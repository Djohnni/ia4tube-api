...
Ressalva de revisão posterior: os registros antigos de limpeza não comprovavam a ordem segura em caso de tarefa ainda ativa. Node executava `after` em ordem FIFO: PostgreSQL/root eram removidos antes do after do host. A prova Linux CI3 revelou um preparo musical legitimamente ainda processando após a janela SDK de 15 s; a asserção de sucesso imediato falhava e essa limpeza prematura removia seus recibos. A fixture foi corrigida para SDK start sempre assíncrono, reconciliação explícita da mesma execução dentro do deadline original e disposers dependentes antes de qualquer fechamento/removal do PG. O shutdown agora exige prova nativa de zero descendentes de todas as execuções locais, bloqueia novos comandos run e confirma PG/root ainda disponíveis antes de fechar a ponte. Se o encerramento não for comprovado, a raiz/banco sintéticos são preservados e o teste falha; nunca se apaga em um finally incondicional.

Isso não explica automaticamente os erros Windows anteriores de spawn, ocorridos antes do teardown. Além disso, o antigo diagnóstico `f.executor.observe` apontava para o executor local da API, não para o remoto: `receipt_missing` dessa leitura não comprovava perda no host Workflow. O diagnóstico foi separado e o host pode emitir apenas uma categoria fechada do erro de spawn, sem mensagem, caminho ou argumentos. Nenhum timeout SDK/nativo foi ampliado para fazer o teste passar.
