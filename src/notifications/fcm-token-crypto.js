...
function validateStoredRecord(record) {
  ...
  if (
    !ciphertext.length ||
    encodeBase64Url(ciphertext) !== ciphertextValue
  ) {
    fail("fcm_token_record_invalid");
  }
...
function timingSafeEncodedEqual(left, right) {
  try {
    const leftBuffer = validateFingerprint(left);
    const rightBuffer = validateFingerprint(right);
...
function createFcmTokenCrypto({
  ...
  if (!KEY_ID_PATTERN.test(activeKeyId)) {
    fail(
      activeKeyId
        ? "fcm_token_key_id_invalid"
        : "fcm_token_key_id_missing"
    );
  }
  ...
  if (
    !encryptionKeys.has(activeKeyId) ||
    !hmacKeys.has(activeKeyId)
  ) {
  ...
  function decryptToken(record) {
    ...
    try {
      ...
      const token = validateToken(plaintext);
      const expectedFingerprint = fingerprintToken(token, validated.keyId);
      if (
      ...
      if (
        ...
        error.code !== "fcm_token_record_invalid"
      ) {
        throw error;
      }
      fail("fcm_token_decrypt_failed");
  ...
  function recordMatchesToken(record, token) {
    const validated = validateStoredRecord(record);
    if (!hmacKeys.has(validated.keyId)) {
      fail("fcm_token_key_unavailable");
    }
    const candidate = fingerprintToken(token, validated.keyId);
