...
function idempotencyKey({
  ...
  return crypto
    ...
    .update(JSON.stringify([
      validateSafeId(eventType, "art_ready_event_type_invalid"),
      validateSafeId(generationId, "art_ready_generation_id_invalid"),
...
function validateEvent(key, event) {
  ...
  if (
    ...
    typeof event !== "object" ||
    Array.isArray(event) ||
    Object.keys(event).some((field) => !allowedFields.has(field)) ||
...
function createArtReadyOutbox({
  ...
  function get({ generationId, tokenFingerprint }) {
    const key = idempotencyKey({ generationId, tokenFingerprint });
  ...
  function transition({
    ...
    if (previous?.state === "prepared" && state === "prepared") return previous;
    if (previous?.state === "failed" && state === "prepared") return previous;
