...
function printUsageAndExit() {
  console.log("IA4Tube Mobile V1 - teste de preview do pedido");
  console.log("");
  console.log("Uso:");
  console.log("  TOKEN=\"seu_jwt\" PEDIDO_ID=\"20260525_180819\" node app_mobile/tests/mobile_order_preview_test.js");
  console.log("");
  ...
  console.log("  node app_mobile\\tests\\mobile_order_preview_test.js");
  console.log("");
  process.exit(1);
}
...
async function main() {
  ...
  const url = `${API_BASE}/pedidos/${encodeURIComponent(PEDIDO_ID)}/preview`;
  console.log(`GET ${url}`);
  console.log("");
  ...
  const outputPath = path.join(outputDir, `preview_${safeId(PEDIDO_ID)}${ext}`);
...
main().catch((err) => {
  console.error("Erro ao consultar preview:");
