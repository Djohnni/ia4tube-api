...
const BRANCH =
  "social/checkpoint-3b0-exact-0004-runner-linux-tap-binding-fix-20260814";
const BASE = "13e38b875db2a220514fe06113663c517c975592";
const SOURCE_COMMIT = "8534817574a22dbd144a835c9f3585c44ee11c96";
const MESSAGE =
  "[run-social-3b0] force TAP reporter and bind exact 0003 migration";
const IMAGE =
...
const RUNNER_LF_SHA256 =
  "70bbc4fd9e34d7fda35b0e4213640c0c645027ece796c6dfe2196177c4a11e8a";
const RUNNER_FILTERED_OID = "c2c4937f552f6e1f44f64867ca6697729b06d955";
const RUNNER_LF_BYTES = 36_196;
...
const REAL_TEST_FILTERED_OID = "9d87dfa5ec0b69a3a6b54bcee428b87078d526ad";
const REAL_TEST_LF_BYTES = 129_897;
...
test("functional pins come from canonical Git blobs after path clean filters", () => {
  ...
  assert.equal(currentBytes.length, REAL_TEST_LF_BYTES);
  assert.equal(
    crypto.createHash("sha256").update(currentBytes).digest("hex"),
    REAL_TEST_LF_SHA256
  );
  assert.equal(pins[REAL_TEST_FILE], REAL_TEST_LF_SHA256);
  ...
  assert.deepEqual(
    Buffer.from(currentSource.replace(REAL_TEST_BINDING_LINE, ""), "utf8"),
    historicalBytes
  );
