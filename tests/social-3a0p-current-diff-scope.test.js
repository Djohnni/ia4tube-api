...
function runMandatoryContractProofs() {
  ...
  // 1. Local mode recognizes exactly the thirteen authorized paths.
  proof(() => {
  ...
  // 2. Post-commit mode recognizes exactly the same thirteen paths.
  proof(() => {
  ...
  // 5. Exact11, Exact12 and Exact14 changed-path inventories are refused.
  proof(() => {
    for (const unstagedTrackedFiles of [
      ...
      [
        ...LOCAL_UNSTAGED_TRACKED_FILES,
        "tests/fourteenth-scope-path.test.js"
      ]
    ]) {
      assert.throws(
        () => assertRouteInventory(makeLocalSnapshot({
...
test("a barreira do runner exato 0004 contem exatamente os treze caminhos autorizados", () => {
  const result = assertRouteInventory(sharedSnapshotCache.read());
  assert.equal(
    ROUTE_BRANCH,
    "social/checkpoint-3b0-exact-0004-runner-linux-lf-pins-20260814"
  );
  assert.equal(AUTHORIZED_CHANGED_FILES.length, 13);
  assert.deepEqual(result.files, [...AUTHORIZED_CHANGED_FILES].sort());
  assert.equal(
