...
function makeLocalSnapshot(overrides = {}) {
  return {
    ...
    routeCommittedFiles: [...PARENT_COMMITTED_FILES],
    functionalCommittedFiles: [],
    stagedFiles: [],
...
function createCapturedSuccessfulSpawn() {
  ...
  const outputs = [
    ...
    encodeNulPaths(PARENT_COMMITTED_FILES),
    Buffer.alloc(0),
    Buffer.alloc(0),
...
function runMandatoryContractProofs() {
  ...
  // 1. Local mode recognizes exactly the twenty authorized paths.
  proof(() => {
  ...
  // 2. Post-commit mode recognizes exactly the same twenty paths.
  proof(() => {
  ...
  // 5. Incomplete Incremental5 inventories and any sixth path are refused.
  proof(() => {
    for (const unstagedTrackedFiles of [
      LOCAL_UNSTAGED_TRACKED_FILES.slice(0, 3),
      LOCAL_UNSTAGED_TRACKED_FILES.slice(0, 4),
      [
        ...LOCAL_UNSTAGED_TRACKED_FILES,
        "tests/sixth-incremental-path.test.js"
      ]
    ]) {
      assert.throws(
        () => assertRouteInventory(makeLocalSnapshot({
...
test("a barreira do runner exato 0004 contem exatamente os vinte caminhos autorizados", {
  ...
  assert.equal(
    ROUTE_PARENT_COMMIT,
    "70011da3f470f7f2f14352d2e087f32cea7873fa"
  );
  assert.equal(
    ROUTE_BASE_COMMIT,
  ...
  assert.equal(INCREMENTAL_CHANGED_FILES.length, 5);
  assert.equal(new Set(INCREMENTAL_CHANGED_FILES).size, 5);
