...
test("server boot refuses privileged credentials and preparation enablement before startup", () => {
  ...
  // This production preparation entry point rejects any enablement before
  // parsing runtime credentials or creating a pool. The lower-level parser's
  // exact-login mismatch remains independently tested above.
  assert.match(
    runtimeOutput,
    /social_production_preparation_incomplete/
  );
  ...
  for (const output of [privilegedOutput, operatorSecretOutput, runtimeOutput]) {
    assert.equal(output.includes("API rodando na porta"), false);
  }
});
