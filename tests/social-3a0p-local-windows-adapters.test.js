...
function gateResults() {
  return {
    ...
    backupRestore: async () => ({ physicalExecution: true, syntheticOnly: true, profile0003: true, profile0004: true, restoreIsolated: true, manifestTamperRefused: true, crossProfileRefused: true, operationalRollback: true, disposableRemoved: true, fileFsync: true, bundleSize: 123, bundleSha256: BUNDLE_HASH }),
    async destroy() {}
  };
}
...
function fixture(overrides = {}) {
  ...
  const archivePath = path.join(ownedRoot, "postgresql-18.4.zip");
  const calls = {
    process: [],
    queries: [],
    pools: [],
  ...
  class FakePool {
    constructor(options) { this.options = options; calls.pools.push(options); }
    async connect() {
      return {
  ...
  const input = {
    ...
    packageDescriptor: { archivePath, expectedSha256: HASH, version: POSTGRES_VERSION },
...
test("adapter executes the complete physical lifecycle with injected fakes only", async () => {
  const { adapters, calls, input, ownedRoot } = fixture();
  ...
  const pendingPath = `${ownedRoot}-evidence.json.pending`;
  const finalPath = `${ownedRoot}-evidence.json`;
...
test("the production adapter supplies concrete connector gates by default", () => {
...
test("package hash and archive traversal are rejected before extraction", async () => {
  ...
  await assert.rejects(badHash.adapters.validatePackage(badHash.input), { code: "windows_harness_archive_sha256_mismatch" });
...
test("cleanup failure leaves only explicitly pending evidence", async () => {
  ...
  const finalPath = `${base.ownedRoot}-evidence.json`;
...
test("failure before evidence collection cannot create final physical evidence", async () => {
  ...
  assert.equal(base.calls.files.has(`${base.ownedRoot}-evidence.json`), false);
  assert.equal(base.calls.files.has(`${base.ownedRoot}-evidence.json.pending`), false);
});
...
test("controller termination covers every active harness child PID", async () => {
  ...
  for (const pid of [5101, 5102, 5103, 5104]) journal.registerProcess(pid);
...
test("ambiguous start preserves the owned root when PID and compensation are unconfirmed", async () => {
  ...
  const base = fixture({ dependencies: {
    ...
    systemProbe: {
      async assertClean() { return !startAttempted; },
