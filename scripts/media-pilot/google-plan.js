...
function createOperationalPlan({ imageId, operatorIpv4, packageSha256, packageReviewSha256, authorizationSha256,
  ownerCompanyId, ownerUserId, workerId, finance, maxExistenceSeconds = 7200, admissionSeconds = null, schema = null } = {}) {
  ...
  // Contract version and mission duration are independent. Keep the old
  // default for immutable historical plans; new two-hour callers request v2.
  if (schema === null) schema = extended ? 2 : 1;
  if (![1, 2].includes(schema) || extended && schema !== 2) fail("schema_invalid");
  const currentFinance = schema === 2;
  ...
  if (!finance || Object.keys(finance).sort().join() !== (currentFinance ? extendedKeys : legacyKeys) ||
  ...
  if (currentFinance) {
    if (finance.historicalReserveIsObservedExpense !== false || !Number.isFinite(finance.historicalPlanningReserveUsd) ||
        ...
        finance.alreadyIncurredObservedAt !== null && (!Number.isSafeInteger(finance.alreadyIncurredObservedAt) ||
          finance.alreadyIncurredObservedAt < 1 || extended && finance.alreadyIncurredObservedAt > finance.verifiedAt)) fail("finance_invalid");
  ...
  const priorBudgetBasisUsd = currentFinance ? Math.max(finance.alreadyIncurredUsd ?? 0, finance.historicalPlanningReserveUsd) : finance.alreadyIncurredUsd;
  const priorBudgetBasisKind = currentFinance ? finance.alreadyIncurredUsd !== null && finance.alreadyIncurredUsd >= finance.historicalPlanningReserveUsd ?
  ...
  const value = { schema, kind: "ia4tube-google-owner-operational-pilot", infrastructure,
    ...
    finance: { ...finance, estimatedInfrastructureUsd, estimatedMaximumUsd,
      ...(currentFinance ? { priorBudgetBasisUsd, priorBudgetBasisKind, estimatedPlanningTotalUsd } : {}), invoiceCapGuaranteed: false } };
...
function validateOperationalPlan(plan, { now = null } = {}) {
  ...
  const expected = createOperationalPlan({ imageId: plan?.infrastructure?.sourceImageId, operatorIpv4: plan?.infrastructure?.operatorIpv4,
    ...
    maxExistenceSeconds: plan?.maxExistenceSeconds, admissionSeconds: plan?.admissionSeconds, schema: plan?.schema });
  ...
  if (now !== null && f.alreadyIncurredObservedAt != null && f.alreadyIncurredObservedAt > now) fail("finance_invalid");
  return plan;
}
