...
Antes de carregar `pg` ou conectar, o operador exige comando exato, aprovação de inspeção, URL exata, TLS estrito e os seis checksums locais. URL exige usuário/senha, porta explícita e somente `sslmode=verify-full`; rejeita outro banco, host, parâmetro ou fragmento. Não há override de destino na CLI.
...
- Presença, tipo ordinário e privilégio de leitura do ledger e do marcador. Só os metadados conhecidos e legíveis são lidos, limitados a sete linhas do ledger e duas do marcador. Nenhuma tabela de clientes, mídia, tokens ou publicações é consultada.
- Se houver ledger legível, prefixo exato e checksums 0001–0006; se houver marcador legível, ambiente `production` e formato UUID. O UUID não é emitido e **não** é comparado com uma identidade previamente aprovada nesta entrega.
...
O relatório sempre contém `applyAvailable: false` e os bloqueios de recuperação, rota de aplicação, revisão de 0005/0006 e prova comportamental de RLS. Objetos inesperados, privilégios ausentes ou atributos incompatíveis acrescentam bloqueios. Um erro de identidade, checksum, marcador, consulta ou encerramento não vira resultado parcial aprovado.

## Checksums e rota ainda não liberada

`db/migrations/checksums.json` e os seis SQL permanecem imutáveis. O operador fixa os mesmos hashes da referência e verifica os **bytes reais**, sem normalizar fim de linha nem substituir checksum. Os SQL canônicos usam LF; uma cópia CRLF deve ser corrigida para os bytes canônicos, nunca aceita por relaxamento do verificador.

Não reutilizar o provisionador de staging: ele fixa identidade de staging, e suas proteções não constituem autorização para produção. O aplicador genérico em `src/persistence/postgres/migrations.js` recusa 0005 pendente em produção e 0006 pendente no fluxo genérico; as rotas dedicadas existentes dependem de alvos/perfis de staging. A preparação de produção precisa de rota própria revisada, com os mesmos SQL/checksums, controles de papel/ownership/ACL/RLS e verificação do catálogo final. Este preflight não contorna esses bloqueios e não chama nenhum aplicador.
...
4. Para baseline realmente vazio, revisar uma captura/restauração explícita desse perfil: o operador de backup social existente usa perfis a partir de 0003, não perfil zero. Não fingir que um baseline vazio satisfaz o manifesto de um esquema 0003–0006. Para conteúdo existente, usar o perfil exato e comprovar compatibilidade de catálogo/ledger antes de exportar.
...
6. Só após a prova de recuperação e a revisão independente da rota 0001–0006, executar a aplicação necessária na identidade de produção sob a autorização condicionada já recebida. Não pedir novamente autorização para os mesmos seis arquivos. Uma extensão além deles depende de autorização específica. Não trocar configuração do webservice, fazer deploy, OAuth ou publicação como consequência desta inspeção.
